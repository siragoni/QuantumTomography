...
// opt = 1; 2018 r
{
  ...
  Bool_t gridTest = kFALSE;
...
#if !defined (__CINT__) || defined (__CLING__)
    gInterpreter->LoadMacro("AliAnalysisTaskPbPbTree.cxx++g");
    AliAnalysisTaskPbPbTree *task = reinterpret_cast<AliAnalysisTaskPbPbTree*>(gInterpreter->ExecuteMacro("AddTaskPbPbTree.C"));
#else
    gROOT->LoadMacro("AliAnalysisTaskPbPbTree.cxx++g");
    gROOT->LoadMacro("AddTaskPbPbTree.C");
    AliAnalysisTaskPbPbTree *task = AddTaskUPCforwardpPb();
#endif
    ...
    if(local) {
      ...
      alienHandler->SetAdditionalLibs("AliAnalysisTaskPbPbTree.cxx AliAnalysisTaskPbPbTree.h");
      alienHandler->SetAnalysisSource("AliAnalysisTaskPbPbTree.cxx");
      ...
      if (opt == 0) {
        ...
        alienHandler->SetGridWorkingDir("PbPbLHC15o_QT_complete");
